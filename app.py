...
# --- 2. DATA MAPPING (DEFINED FIRST TO AVOID ERROR) ---
MONTH_MAP_ID = {
...
def load_gsheets_data():
    ...
    try:
        ...
        # Parse Periode for YtD
        def parse_date(p):
            p = str(p).replace('-', ' ').strip()
            parts = p.split(' ')
            m = MONTH_MAP_REV.get(parts[0], 0)
            y = parts[1] if len(parts) > 1 else "2026"
            return pd.Series([m, y])

        df_f[['m_idx', 'year_val']] = df_f['Periode'].apply(parse_date)
        for df in [df_f, df_l]:
...
with tab1:
    ...
    if not df_f.empty:
        ...
        # YtD CALCULATION: Kunci Jan-Sel_Month
        ...
        st.warning(f"Data untuk periode {s_m_name} {s_y_val} tidak ditemukan.")

# ==========================================
# TAB 2 & 3 (LOCKED & ACCURATE)
# ==========================================
with tab2:
    ...
    if not df_l.empty:
        ...
        st.divider()
        st.plotly_chart(px.bar(df_l.groupby('Kreditur')['Nominal'].sum().reset_index().sort_values('Nominal', ascending=False), x='Kreditur', y='Nominal', text_auto=',.0f', color='Kreditur'), use_container_width=True)
