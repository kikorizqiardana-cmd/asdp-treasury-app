...
def fetch_global_market_data_v2():
    ...
    data_results = {key: {'val': 0.0, 'pct': 0.0, 'hist': None} for key in tickers.keys()}
...
with tab4:
    ...
    render_sparkline_widget(idx1, "🇮🇩 IHSG (Jakarta)", 'IHSG', prefix="")
    render_sparkline_widget(idx2, "🇺🇸 S&P 500 (Wall St)", 'S&P 500', prefix="")
    render_sparkline_widget(idx3, "🇬🇧 FTSE 100 (London)", 'FTSE 100', prefix="")
    
    st.divider()
    
    st.subheader("💵 Forex & Commodities")
    fx1, fx2, fx3, oil = st.columns(4)
    render_sparkline_widget(fx1, "🇺🇸 USD / IDR", 'USD/IDR', prefix="Rp ", is_inverse=True)
    render_sparkline_widget(fx2, "🇪🇺 EUR / IDR", 'EUR/IDR', prefix="Rp ", is_inverse=True)
    render_sparkline_widget(fx3, "🇯🇵 JPY / IDR", 'JPY/IDR', prefix="Rp ", is_inverse=True)
